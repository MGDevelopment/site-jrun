	           ...
	           <table width="2" border="0" cellspacing="0" cellpadding="0" class="divComprar">
	             <tr>
	               <td height="19" class="divInfo">
		               <a 	class="FProductos"
		               		href="<%=CategoriaService.getURL(articulos[i].getCategoria())+ArticuloManager.getURL(articulos[i])%>"
		               		alt="<%=Convert.corregir(articulos[i].getTitulo(), true).toUpperCase()%>">
			               <img
			               src="/imagenes/<%=Globals.seccion(idSeccion)%>/b-info.gif"
			               alt="<%=(articulos[i].getAtributoPrincipal()!= null)?articulos[i].getAtributoPrincipal() + " - ":""%><%=Convert.corregir(articulos[i].getTitulo(), true).toUpperCase()%>" title="<%=(articulos[i].getAtributoPrincipal()!= null)?articulos[i].getAtributoPrincipal() + " - ":""%><%=Convert.corregir(articulos[i].getTitulo(), true)%>" border="0" />
		               </a>
	               </td>
	               <td class="divComprarPedir"><%if (DisponibilidadDAO.buscaDisponibilidad(articulos[i].getIdDisponibilidad()).estaDisponible() && articulos[i].getHabilitadoTematika().equals("S")) { %><a href="javascript:carrito_AgregarArticulo(<%=articulos[i].getIdArticulo()%>);window.scrollTo(0,0);" rel="nofollow"><img src="/imagenes/<%=Globals.seccion(idSeccion)%>/b-comprar.gif" alt="Comprar"  border="0" /></a><%}else{%><a href="javascript:if (confirm('Este producto no esta disponible actualmente. Desea hacer un pedido?'))document.location = '/PedidoEspecial?ID_ARTICULO=<%=articulos[i].getIdArticulo()%>';" rel="nofollow"><img src="/imagenes/<%=Globals.seccion(idSeccion)%>/b-pedir.gif" alt="Comprar"  border="0"/></a><% } %></td>
	             </tr>
	           </table>
	         </div>
         </td>
