...
<html>
				...
				$('tblMensajeLoad').style.display = 'none';
	  			try {
					...
					var i;
						...
						document.frmOrdenes.btnConsultar.style.display = 'block';
					} else {
				...
				$('tblMensajeLoad').style.display = 'none';
	  			try {
					...
					if (obj.ordenes.length > 0) {
						...
						for (i=0; i<obj.ordenes.length; i++) {
							...
							if (obj.ordenes[i].estado == '<%=OrdenService.ESTADO_APROBADO%>') {
								ordenesConCambios++;
								datos[5] = "<input name='estado" + ordenesConCambios  + "' type='radio' value='A'>";
								datos[7] = "<input name='estado" + ordenesConCambios  + "' type='radio' value='P' checked>";
								datos[0] = datos[0] + "<input type='hidden' name='orden" + ordenesConCambios + "' value ='" + obj.ordenes[i].id_orden + "' >"
								datos[4] = "<div style='" + estadoAprobado + "'><b>" +  obj.ordenes[i].estado + "</b></div>";
							} else if (obj.ordenes[i].estado == '<%=OrdenService.ESTADO_VENCIDA%>' || obj.ordenes[i].estado == '<%=OrdenService.ESTADO_CANCELADA%>') {
 								ordenesConCambios++;
								datos[6] = "<input name='estado"  + ordenesConCambios + "' type='radio' value='R'>";
								datos[7] = "<input name='estado" + ordenesConCambios  + "' type='radio' value='P' checked>";
								datos[0] = datos[0] + "<input type='hidden' name='orden" + ordenesConCambios + "' value ='" + obj.ordenes[i].id_orden + "' >"
								datos[4] = "<div style='" + estadoVencido + "'><b>" +  obj.ordenes[i].estado + "</b></div>";
								...
								datos[7] = "<input name='estado" + ordenesConCambios  + "' type='radio' value='P' checked>";
								datos[0] = datos[0] + "<input type='hidden' name='orden" + ordenesConCambios + "' value ='" + obj.ordenes[i].id_orden + "' >"
								datos[4] = "<div style='" + estadoCargoDiferente + "'><b>" +  obj.ordenes[i].estado + "</b></div>";
							} else {
								ordenesConCambios++;
								datos[0] = datos[0] + "<input type='hidden' name='orden" + ordenesConCambios + "' value ='" + obj.ordenes[i].id_orden + "' >"
								datos[4] = "<div style='" + estadoSinInfo + "'><b>" +  obj.ordenes[i].estado + "</b></div>";
								datos[6] = "<input name='estado"  + ordenesConCambios + "' type='radio' value='R'>";
								datos[7] = "<input name='estado" + ordenesConCambios  + "' type='radio' value='P' checked>";
							}
					...
					if (ordenesConCambios > 0) {
						...
						frm.btnConsultar.style.display = 'none';
					}
				} catch (e) {
					try {
						$('divMensajeError').innerHTML = obj.Resultado.mensaje[0];
	...
	function getOrdenByEstadoMedio() {
		...
		var valores = 'idEstado=' + "'2'"
			+ '&idMedioCobro=' +  "'<%=Globals.CLAVE_MEDIO_DE_COBRO_DINERO_MAIL%>'";
	...
	function consultarADineroMail() {
		...
		var valores = 'idEstado=' + "'2'"
			+ '&idMedioCobro=' +  "'<%=Globals.CLAVE_MEDIO_DE_COBRO_DINERO_MAIL%>'";
		ejecutarAjax('/ConsultarOrdenDMADM?param=' + Math.random(), valores
			...
			<table cellpadding="0" cellspacing="0" >
				<tr>
					<td width="1005">
						...
						<br>
						<table align="center" border="1" cellpadding="2" cellspacing="0" style="border-collapse: collapse; border: 2px solid; border-color: #5AB5DE;" width="752" id="tblOrden"><tbody><tr bgcolor="#59B3D9"><td width="50px"><b>Orden</b></td><td><b>Medio de Pago</b></td><td width="70px"><b>Fecha</b></td><td width="50px" align="right"><b>Total</b></td><td  width="120px"><b>Estado</b></td><td align="center"><b>Aprobar</b></td><td align="center"><b>Rechazar</b></td><td align="center"><b>Pendiente</b></td></tr></tbody></table>
						<br>
