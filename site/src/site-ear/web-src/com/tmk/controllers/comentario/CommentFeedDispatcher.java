...
import com.tmk.generacion.feed.ComentarioFeed;
...
public class CommentFeedDispatcher extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			if (mail != null) {
				String fileName = mail.toLowerCase() + "_" + page;

				///CORREGIR EL PATH
				File file = new File(Contenido.getServletContext().getRealPath(MainHelper.RES_DIR_CONTENIDOS_ESTATICOS + MainHelper.RES_DIR_COMMENT_FEED )
						 + "/" + fileName + ".xml");
				if (file.exists()) {
						...
						try {
							...
							try {
								...
								try {
									...
									feed.setId("http://www.tematika.com/feed/userid/" + mail + "/0/comments.htm");
									Title title = new Title("Comentarios de " +  mail, "text");
									feed.setTitle(title);
									Link link = new Link("self", "application/atom+xml", "http://www.tematika.com/feed/userid/" + mail + "/0/comments.htm");
									feed.setLink(link);
									feed.setUpdated(new Date(socio.getFAlta().getTime()));
									...
									feed.setOpenSearch_itemsPerPage(new Integer(new Double(ComentarioFeed.itemsPerPage).intValue()));
