...
public class WishListsFeedDispatcher extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			if (mail != null) {
				String fileName = "indice_" + mail + "_" + page;
						...
						try {
							...
							try {
								...
								try {
									...
									feed.setId("http://www.tematika.com/feed/userid/" + mail + "/" + 0 + "/wishlists.htm");
									Title title = new Title("Listas de Deseos de " +  mail, "text");
									feed.setTitle(title);
									Link link = new Link("self", "application/atom+xml", "http://www.tematika.com/feed/userid/" + mail + "/" + 0 + "/wishlists.htm");
									feed.setLink(link);
