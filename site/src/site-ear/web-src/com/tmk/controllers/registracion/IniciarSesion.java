...
import com.tmk.view.model.LoginForm;
...
public class IniciarSesion extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //modelo de login
    	try {
			loginForm = (LoginForm)ServiceLocator.getModeloBuilderService().getModelo(LoginForm.class, request);			
		} catch (Exception e1) {			
			res = new Resultado(false,new String[]{"Error interno, por favor reintente."},null);
			response.getWriter().print(xstream.toXML(res));
			return;
		}
		//algun campo vacio?
    	if(loginForm.getLogin()==null || loginForm.getPassword()==null) {
    		res = new Resultado(false,new String[]{"Complete usuario y contraseña."},null);
			response.getWriter().print(xstream.toXML(res));
			return;
    	}
    	//ya esta logueado?
    	if (request.getSession().getAttribute("Registracion.socioPK") != null) {
    		res = new Resultado(true,new String[]{"Ya esta loguado, redireccionando..."},null);
        ...
        if (socio == null) {
			...
			try {
	            ...
	            res = new Resultado(false,new String[]{"LA CLAVE O NOMBRE DE USUARIO ES INCORRECTO."},null);
	            response.getWriter().print(xstream.toXML(res));
	            return;
			} catch (Exception e) {
				res = new Resultado(false,new String[]{"ERROR INTERNO, POR FAVOR INTENTA EN UN MOMENTO."},null);
	            response.getWriter().print(xstream.toXML(res));
	            return;
			}		
        } else {
	    ...
	    res = new Resultado(true,new String[]{"sesion aprobada"},null);
	    res.setTargetRedirect(_DISPATCHER);
	    response.getWriter().print(xstream.toXML(res));
    }
