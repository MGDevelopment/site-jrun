...
abstract public class BusquedaGenerica_ACTUAL {
	...
	public BusquedaGenerica_ACTUAL(String texto, Integer seccion,
	...
	*/
		...
		if (filtroValor != null) {
			...
			for(int i=0;i<inCondicion.length;i++) {
		        ...
		        if(this.seccionBuscada.equals("0")){
		        	sql.append(" and categoria_seccion in(1,3,4,5)");
		        }else{
		        	sql.append(" and categoria_seccion = ");
	...
	public static int[] getTiempos(String nombreBuscador) {
		int []retorno =  new int[(BusquedaGenerica_ACTUAL.SEGUNDOS_TIMEOUT * Globals.SECCIONES.length) + 1];
		Enumeration<String> keys = estadisticasDeBusqueda.keys();
		while (keys.hasMoreElements()) {
			Object nombre = keys.nextElement();
