...
public class BusquedaAvanzada extends BusquedaGenerica {
	...
	public String getQuerySubtotales() {
		StringBuffer sql = new StringBuffer();
		sql.append(Globals.ENTER).append(" SELECT count(*) cantidad ");
	...
	public StringBuffer getQueryParcial(Filtro filtro) {
		StringBuffer sql = new StringBuffer();
		sql.append("select * from (");
		if (isbn != null) sql.append(" INTERSECT ").append(getQueryISBN(filtro));
    	if (titulo != null) sql.append(" INTERSECT ").append(getQueryTitulo(filtro));
    	if (autor != null) sql.append(" INTERSECT ").append(getQueryAutor(filtro));
		if (editorial != null) sql.append(" INTERSECT ").append(getQueryEditorial(filtro));
		if (palabrasClaves != null) sql.append(" INTERSECT ").append(getQueryPalabrasClaves(filtro));
		sql.append(Globals.ENTER).append(" )");
		//sql.append((criterio == null) ? "" : criterio.getTextoQuery());
		sql = new StringBuffer(sql.toString().replaceFirst("INTERSECT", ""));
		return sql;
