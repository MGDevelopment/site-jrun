...
public class CopyOfResultadoBusquedaAction extends Action {
			...
			*/
			...
			if((filtroValor != null || request.getParameter("mantenerIds")!=null) && arrayIn!=null) {
				...
				if(busqueda.getItems().size() > 1000) { 
					arrayIn = Comparador.listaToArrayIN(busqueda.getItems(),"id_articulo");
				...
				if(resultadoBusquedaForm.getEsBusquedaDeInicio() && filtroValor==null) {
					...
					for(int i=1;i<=5;i++) {
						if(busqueda.getCantPorSecciones()[0][i] > 0) {
							...
							//uso A;B;C;D para que se carguen en orden y no aplicar logica de mas
	...
	public static String getTexto(String texto) {
		...
		if (auxTxt.length()>0) {
			...
			auxTxt = auxTxt.replaceAll("[^a-zA-Zá-úÁ-Ú&&\\S]", "");
