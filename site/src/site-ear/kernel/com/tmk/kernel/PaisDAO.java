/**
 * $Log: PaisDAO.java,v $
 * Revision 1.24  2004/06/15 20:56:17  oGPistoia
 ...
 *
 * Revision 1.19  2003/11/07 15:32:59  GPistoia
 ...
 *
 * Revision 1.18  2003/10/12 22:11:26  GPistoia
 ...
 * -Mails configurables.
 *
 * Revision 1.17  2003/10/07 14:52:19  GPistoia
 ...
 *
 * Revision 1.10  2003/09/08 15:24:47  GPistoia
 ...
 *
 * Revision 1.8  2003/09/05 19:56:29  GPistoia
 * -Nuevos parametros
 * -Division de GPay para pago con fax
 * -Configuracion modificada de archivos del server
 *
 * Revision 1.7  2003/08/29 17:54:23  GPistoia
 ...
 *
 * Revision 1.6  2003/08/27 18:43:52  GPistoia
 * -Comienzo Fraude
 *
 * Revision 1.5  2003/08/12 16:25:28  GPistoia
 * -Cierre de proceso de compra pre-produccion
 *
 * Revision 1.4  2003/08/06 21:28:22  GPistoia
 ...
 *
 * Revision 1.3  2003/08/02 16:58:13  GPistoia
...
public class PaisDAO {
	...
	public PaisDAO(int idPais, String nombre) {
		super();
		this.idPais = idPais;
	...
	public synchronized static void clearCaches() {
		if (Globals.PAISES != null) {
			for (int i = 0; i < Globals.PAISES.length; i++) {
				Globals.PAISES[i].clearCache();
			}
		}
	}
	...
	public ProvinciaDAO[] getProvincias() {
		if (provincias == null) {
			Vector temp = new Vector();
			for (int i = 0; i < Globals.PROVINCIAS.length; i++) {
				ProvinciaDAO provinciaDAO = Globals.PROVINCIAS[i];
				if (provinciaDAO.getIdPais() == idPais) {
					temp.add(provinciaDAO);
				}
			}
			provincias = (ProvinciaDAO[]) temp.toArray(new ProvinciaDAO[temp.size()]);
		}
		return provincias;
	}

	public ProvinciaDAO getProvincia(Integer provincia) {
		return (provincia == null) ? getOtraProvincia() : getProvincia(provincia.intValue());
	}

	public ProvinciaDAO getProvincia(int provincia) {
		ProvinciaDAO temp[] = getProvincias();
		for (int i = 0; i < temp.length; i++) {
			ProvinciaDAO provinciaDAO = temp[i];
			if (provinciaDAO.getId() == provincia) {
				return provinciaDAO;
			}
		}
		return getOtraProvincia();
	}

	private ProvinciaDAO getOtraProvincia() {
		return new ProvinciaDAO(idPais, Globals.CODIGO_OTRA_PROVINCIA, "PROVINCIA NO ESPECIFICADA");
	}

	public static final PaisDAO getPais(int clave) {
		for (int i = 0; i < Globals.PAISES.length; i++) {
			if (Globals.PAISES[i].getId() == clave) {
				return Globals.PAISES[i];
			}
		}
		return new PaisDAO(clave, "PAIS NO ESPECIFICADO");
	}

	public static final PaisDAO getPais(Integer clave) {
		return (clave == null) ? Globals.ARGENTINA : getPais(clave.intValue());
	}

	public boolean esArgentina() {
		return (idPais == Globals.ARGENTINA.getId());
	}

	public String toString() {
		return "Pais (" + idPais + ") " + nombre;
	}

	public boolean equals(Object other) {
		return (this == other) ||
		        ((other instanceof PaisDAO) &&
		        (idPais == ((PaisDAO) other).idPais));
	}

	static {
		new Daemon(Daemon.CINCO_SEGUNDOS, Daemon.UN_DIA) {
			protected void ejecutarTarea() throws Exception {
				final Vector paises = new Vector();
				DBUtil.getIdDescripcion(
				        "SELECT ID_PAIS, DESCRIPCION FROM PAISES WHERE HABILITADO_TEMATIKA = 'S' ORDER BY DESCRIPCION",
				...
				Globals.PAISES = (PaisDAO[]) paises.toArray(new PaisDAO[paises.size()]);
				PaisDAO.clearCaches();
			}

			protected String getMensaje() {
				return Globals.PAISES.length + " paises.";
			}
