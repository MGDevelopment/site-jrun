...
public class EstudioDAO {
	...
	public EstudioDAO(String id, String nombre) {
		super();
		this.id = id;
	...
	public String toString() {
		return "Estudio (" + id + ") " + nombre;
	}

	public static EstudioDAO get(String key) {
		...
		return Globals.ESTUDIO_PRIMARIO;
	}
	...
	static {
		new Daemon(Daemon.TREINTA_SEGUNDOS, Daemon.UN_DIA) {
			protected void ejecutarTarea() throws Exception {
				...
				EstudioDAO daos[] = new EstudioDAO[temp.size()];
				for (int i = 0; i < temp.size(); i++) {
					Pair peer = (Pair) temp.get(i);
					daos[i] = new EstudioDAO(peer.getValue1().toString(), peer.getValue2().toString());
				}
				Globals.ESTUDIOS = daos;
			}

			protected String getMensaje() {
				return Globals.ESTUDIOS.length + " estudios cursados.";
			}
