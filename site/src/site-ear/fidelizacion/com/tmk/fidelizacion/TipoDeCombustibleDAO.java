/**
 * $Log: TipoDeCombustibleDAO.java,v $
...
public class TipoDeCombustibleDAO {

	private String id;
	private String nombre;

	public TipoDeCombustibleDAO(String id, String nombre) {
		super();
		this.id = id;
	...
	public String toString() {
		return "Tipo de combustible (" + id + ") " + nombre;
	}

	public static TipoDeCombustibleDAO get(String key) {
		...
		return Globals.TIPO_DE_COMBUSTIBLE_DEFECTO;
	}

	public static String getDomain() {
		return "FDN_TIPO_COMBUSTIBLE";
	}
	...
	static {
		new Daemon(Daemon.TREINTA_SEGUNDOS, Daemon.UN_DIA) {
			protected void ejecutarTarea() throws Exception {
				...
				TipoDeCombustibleDAO daos[] = new TipoDeCombustibleDAO[temp.size()];
				for (int i = 0; i < temp.size(); i++) {
					Pair peer = (Pair) temp.get(i);
					daos[i] = new TipoDeCombustibleDAO(peer.getValue1().toString(), peer.getValue2().toString());
				}
				Globals.TIPOS_DE_COMBUSTIBLE = daos;
			}

			protected String getMensaje() {
				return Globals.TIPOS_DE_COMBUSTIBLE.length + " tipos de combustible.";
			}
