/**
 * $Log: ReporteDeReferenteComprasDaemon.java,v $
 * Revision 1.4  2006/10/12 14:59:05  omsartori
 * no message
 *
...
public class ReporteDeReferenteComprasDaemon extends Daemon {
	static public Vector referenteCompras;

		public ReporteDeReferenteComprasDaemon() {
		...
		try {
			referenteCompras = ReporteReferido.referenteCompras();

			MailUtil.sendMail(
					Globals.MAIL_MAILER,
					Globals.MAIL_REPORTE_DE_SOCIOS,
					Globals.NOMBRE_DEL_SITIO + " - Reporte de Referente Compras.",
					"No se genero el listado de Compras de Referentes.",
					"/mailing/referenteCompras.jsp");
			} catch (Exception e) {
				MailUtil.sendMail(
					Globals.MAIL_MAILER,
					Globals.MAIL_WEBMASTER,
					Globals.NOMBRE_DEL_SITIO + " - Reporte de Referente Compras.",
					"No se genero el listado de Compras de Referentes. Error: " + e.getMessage() + e.toString());
			}
		}
		...
		protected boolean pausada() {
			...
			return (Globals.baseDeDatosEnMantenimiento())
				|| (!Globals.esHorarioDeReporte(3, 30, false))
