...
int idAutor = Convert.toNumber(request.getParameter("idAutor"), 0);
...
<td class="Gcentro">
				...
				function plan() {
				    if(http.readyState == 4){
					    ...
					    <tr>
                    		<td align="left">
	           	            <%
		    		            String urlPage = Globals.DOMINIO_SITIO + "/detalle/biografias.jsp?idArticulo=" + articulo.getIdArticulo() + "&idAutor=" + idAutor;
		    		            urlPage = URLEncoder.encode(urlPage,"ISO-8859-1");
          ...
          <%
          	//obtengo los autores/directores/compositores
	        Vector vec = articulo.getAutor();//obtengo e/los autore/s
          	if (vec == null) {
          		vec = articulo.getProtagonista();
          	}
	        String biografias[];
          	if (vec != null) {
          	biografias  = new String[vec.size()];
	        for(int i=0;i< vec.size();i++){
				biografias[i] = ArticuloManager.getBiografia(idArticulo, Integer.parseInt(((String[])vec.get(i))[0]), Contenido.getServletContext());
          	}
	        if (biografias != null) {
		          for (int i=0; i < biografias.length; i++) { %>
		              <tr>
		                <td>
		                	<table width="375" border="0" align="center" cellpadding="0" cellspacing="0" class="modulosmedioDetalle">
			                  ...
			                  <tr class="modulorecomendadosd">
		    		                ...
		    		                <%}else{ %>
			    		                <table width="375" border="0" cellpadding="0" cellspacing="0" class="">
			    		            <%}%>
					                        <tr>
		            			              <td>
			            			              <a name="titulo">
			            			              <%if(i == 0){ %>
		    		        			              <img src="/imagenes/<%=Globals.seccion(articulo.getIdSeccion())%>/t-biografia.gif" alt="Rese&ntilde;a" width="68" height="10" />
		            				              </a>
		            				              <%}%>
		            			              </td>
		                        			</tr>
			                  ...
			                  <%} else { %>
			                  <tr>
		                      		<td width="50" class="titulosceldasDetalle">
			                      		<a name="titulo">
					              		<%if(i == 0){ %>
				                      		<img src="/imagenes/<%=Globals.seccion(articulo.getIdSeccion())%>/t-biografia.gif" alt="Briograf&iacute;a" width="74" height="12" />
			    						</a>
			    						<%}%>
		                      		</td>
			                  </tr>
			                  ...
			                  <tr>
			                  	<%if(vec.size() > 1){%>
									<%if( biografias[i] != null ) {%>
										<td class="modulosinopsis"><h2 class="Ftexto06"><b>
										<%=Convert.corregir(articulo.getTitulo(), true)%> -
										<%out.print( ((String[]) vec.get(i))[1]);%>
									<%}%></b></h2>
		    	               	<%}else{%>
			    	                <td class="modulosinopsis"><h2 class="Ftexto06"><b><%=Convert.corregir(articulo.getTitulo(), true)%> - <%=articulo.getAtributoPrincipal()%></b></h2>
			    	            <%}%>
								<span class="Ftexto06"><%if( biografias[i] != null ){ out.print(biografias[i]); }%></span></td>
		            	      </tr>
						    </table>
						</td>
		              </tr>
	          		<%}//fin del for
				}// cierre del if(biografia!=null)
          	}
				%>
