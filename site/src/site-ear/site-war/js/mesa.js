...
function cargarImgInicial(idCat, arrIMG, arrDatos) {
	...
	for (i=0; i<arrDatos[idCat].length; i++) {
		arrIMG[idCat][i] = "http://img-tmk.tematika.com/tapas/sitio/" + arrDatos[idCat][i].idArticulo + "c0.jpg";
	}
...
function setMesa(idCategoria) {
	...
	for (i=inicio; i<fin; i++) {
		if (datos[idCategoria][i] != undefined) {
			...
			$(aux).get(0).alt = datos[idCategoria][i].altImagen;
			$(aux).get(0).idArticulo = datos[idCategoria][i].idArticulo;
			var auxi;
			try {
				auxi = "#aitm" + idCategoria + "-" + itm;
				$(auxi).get(0).href = "http://www.tematika.com" + datos[idCategoria][i].urlDetalle;
			} catch (e) {}
...
function setArticulo(objArticulo) {
	...
	if (objArticulo.disponibilidad != undefined && objArticulo.disponibilidad.descripcion != undefined) {
	 ...
	 if(objArticulo.cls_altImagen != undefined)
		 $('#urlImagen').get(0).alt=objArticulo.cls_altImagen;	 
