...
%>
	...
	<table align="center" width="390" border="0" cellpadding="0" cellspacing="0" <%=(urlActual.equals("/tiles/elemento/ultimosVisitados.jsp"))? "class=\"moduloayudatop\"" : "class=\"modulosmedio\""%>>
		<tr>
...
%>
	<table align="center"  width="390" border="0" cellpadding="0" cellspacing="0" <%=(urlActual.equals("/tiles/elemneto/ultimosVisitados.jsp"))? "class=\"moduloayudatop\"" : "class=\"modulosmedio\""%>>
		<tr>
			<td class="titulosceldas">
				<img src="/imagenes/<%=Globals.seccion(idSeccion)%>/t-ultimosvisitados.gif" alt="&uuml;ltimos visitados"  />
			</td>
		</tr>
<%
	}
...
<%
  	int cantidad = -1;
   	if (!urlActual.equals("/tiles/elemento/ultimosVisitados.jsp")){
			...
			<td>
				<table align="center" width="386" border="0" cellpadding="0" cellspacing="0" class="Gultimosvitadosprod">
					<tr>
						<td colspan="3">
							<div align="left">
								<a class="FProductos" href="<%=CategoriaService.getURL(articulos[i].getCategoria())+ArticuloManager.getURL(articulos[i])%>"><%=Convert.corregir(articulos[i].getTitulo(), true)%></a>
						         <%
						         ...
						         %>
	        	 				<span class="FProductos"> | <a href="<%=busquedaPorAtributoPrincipal.buscador.salto()%>" class="Fautores0"><%=articulos[i].getAtributoPrincipal().toUpperCase()%></a>
	        	 				</span>
						        <%
						        	}
						        %>
						        ...
						        %>
						        <br>
						        <a href="<%=busquedaPorCategorias.salto()%>" class="FCategorias"><%=Convert.corregir(articulos[i].getGrupo(), true)%></a>
							</div>
						</td>
					</tr>
	   				<tr>
	           					...
	           					<table width="2" border="0" cellspacing="0" cellpadding="0" class="divComprar">
	             					<tr>
	               						<td height="19" class="divInfo">
		               						<a 	class="FProductos" href="<%=CategoriaService.getURL(articulos[i].getCategoria())+ArticuloManager.getURL(articulos[i])%>" alt="<%=Convert.corregir(articulos[i].getTitulo(), true).toUpperCase()%>">
			               						<img src="/imagenes/<%=Globals.seccion(idSeccion)%>/b-info.gif" alt="<%=(articulos[i].getAtributoPrincipal()!= null)?articulos[i].getAtributoPrincipal() + " - ":""%><%=Convert.corregir(articulos[i].getTitulo(), true).toUpperCase()%>" title="<%=(articulos[i].getAtributoPrincipal()!= null)?articulos[i].getAtributoPrincipal() + " - ":""%><%=Convert.corregir(articulos[i].getTitulo(), true)%>" border="0" />
		               						</a>
	               						</td>
...
<%} else { %>
      ...
      <tr>
        <td><div class="GultvisitadosB" align="center">
            <div><img src="/imagenes/<%=(idSeccion==Globals.SECCION_LIBRO)?"libros/ultimosvisitados.gif" : "ultvisitados.gif" %>" height="30" style="margin-top:10px;" alt="" /></div>
