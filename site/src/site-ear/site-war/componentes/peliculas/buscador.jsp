...
function validarBuscador() {
	...
	if(isEmpty(form.<%=BuscadorHelper.CLAVE_DE_BUSQUEDA%>.value)){
		...
		form.<%=BuscadorHelper.CATEGORIA_SECCION%>.value = "<%=Globals.SECCION_PELICULA%>";
	}
...
<table width="570" border="0" align="left" cellpadding="0" cellspacing="0" class="buscadortabla">
	<form name="formBuscador" action="<%=BuscadorHelper.PAGINA_BUSCADOR%>" method="post" onSubmit="return validarBuscador()">
		<tr>
			<td>
				<input type="hidden" name="<%=BuscadorHelper.CATEGORIA_SECCION%>" value="<%=Globals.SECCION_PELICULA%>">
				<table width="534" border="0" align="center" cellpadding="0" cellspacing="0">
                  <tr>
                    <td width="90"><img src="/imagenes/peliculas/t-busqueda.gif" alt="B&uacute;squeda en" width="90" height="13" /></td>
                    <td width="140"><select name="<%=BuscadorHelper.CLAVE_DE_BUSQUEDA%>" onKeyPress="comboSubmit(event)" class="buscadormenu">
										...
										<option value="<%=BuscadorHelper.POR_AUTOR%>" <%=BuscadorHelper.POR_AUTOR.equals(claveDeBusqueda)?"selected":""%>>Director / Actor</option>
										<option value="<%=BuscadorHelper.POR_EDITORIAL%>" <%=BuscadorHelper.POR_EDITORIAL.equals(claveDeBusqueda)?"selected":""%>>Productora</option>
					...
					<td class="buscadorbuscar" width="59"><input type="image" src="/imagenes/peliculas/b-buscar.gif"></td>
                    <td class="buscadorbavanzada"width="53"><% String pagBuscador = "/articulo/buscadorAvanzado.jsp?" + BuscadorHelper.CATEGORIA_SECCION + "=" + Globals.SECCION_PELICULA;%>
                    <a href="<%=pagBuscador%>"><img src="/imagenes/peliculas/b-busqavanzada.gif" alt="B&uacute;squeda Avanzada" width="44" height="19" border="0" /></a></td>
		           </tr>
        	   </table>
        	</td>
       </tr>
